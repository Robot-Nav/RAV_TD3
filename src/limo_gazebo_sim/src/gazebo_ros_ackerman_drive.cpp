...
void GazeboRosAckermanDrive::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf) {
    ...
    this->max_linear_speed_ = 1.0;
    if (_sdf->HasElement("maxLinearSpeed")) {
        this->max_linear_speed_ = _sdf->GetElement("maxLinearSpeed")->Get<double>();
    }

    this->max_angular_speed_ = 1.6;
    if (_sdf->HasElement("maxAngularSpeed")) {
        this->max_angular_speed_ = _sdf->GetElement("maxAngularSpeed")->Get<double>();
    }

    this->max_linear_accel_ = 1.2;
    if (_sdf->HasElement("maxLinearAccel")) {
        this->max_linear_accel_ = _sdf->GetElement("maxLinearAccel")->Get<double>();
    }

    this->max_angular_accel_ = 2.5;
    if (_sdf->HasElement("maxAngularAccel")) {
        this->max_angular_accel_ = _sdf->GetElement("maxAngularAccel")->Get<double>();
    }

    this->max_wheel_speed_ = 2.0;
    if (_sdf->HasElement("maxWheelSpeed")) {
        this->max_wheel_speed_ = _sdf->GetElement("maxWheelSpeed")->Get<double>();
    }

    this->max_steer_rate_ = 1.8;
    if (_sdf->HasElement("maxSteerRate")) {
        this->max_steer_rate_ = _sdf->GetElement("maxSteerRate")->Get<double>();
    }
...
void GazeboRosAckermanDrive::GetWheelVelocities() {
    auto clamp_value = [](double v, double low, double high) {
        return std::max(low, std::min(v, high));
    };

    boost::mutex::scoped_lock scoped_lock(lock);
    const double dt = std::max(1e-4, update_period_ > 0.0 ? update_period_ : 0.01);
    double cmd_v = std::isfinite(x_) ? x_ : 0.0;
    double cmd_w = std::isfinite(rot_) ? rot_ : 0.0;

    cmd_v = clamp_value(cmd_v, -max_linear_speed_, max_linear_speed_);
    cmd_w = clamp_value(cmd_w, -max_angular_speed_, max_angular_speed_);

    const double dv_max = std::max(1e-4, max_linear_accel_ * dt);
    const double dw_max = std::max(1e-4, max_angular_accel_ * dt);
    filtered_x_ += clamp_value(cmd_v - filtered_x_, -dv_max, dv_max);
    filtered_rot_ += clamp_value(cmd_w - filtered_rot_, -dw_max, dw_max);

    const double vr = filtered_x_;
    const double va = filtered_rot_;
    double left_side_velocity  = vr;
    double right_side_velocity = vr;
    double target_left_steer = 0.0;
    double target_right_steer = 0.0;

    if (std::fabs(vr) > 1e-4 && std::fabs(va) > 1e-4) {
        const double min_radius = track_ * 0.5 + 1e-3;
        const double r = std::max(std::fabs(vr / va), min_radius);
        double steer_cmd = std::atan(wheelbase_ / r);
        steer_cmd = (va < 0.0) ? -steer_cmd : steer_cmd;
        steer_cmd = clamp_value(steer_cmd, -max_steer_angle_central, max_steer_angle_central);

        const int sig = steer_cmd < 0.0 ? -1 : 1;
        ConvertCentralAngleToLeftRight(steer_cmd, r, target_left_steer, target_right_steer);
        left_side_velocity = vr - sig * vr / r * track_ * 0.5;
        right_side_velocity = vr + sig * vr / r * track_ * 0.5;
    }

    const double max_steer_step = std::max(1e-4, max_steer_rate_ * dt);
    last_motor_cmd[0].data += clamp_value(target_right_steer - last_motor_cmd[0].data, -max_steer_step, max_steer_step);
    last_motor_cmd[1].data += clamp_value(target_left_steer - last_motor_cmd[1].data, -max_steer_step, max_steer_step);

    if (std::fabs(vr) > 1e-4) {
        const double fr_cos = std::max(0.2, std::fabs(std::cos(last_motor_cmd[0].data)));
        const double fl_cos = std::max(0.2, std::fabs(std::cos(last_motor_cmd[1].data)));
        wheel_speed_[RIGHT_FRONT] = right_side_velocity / fr_cos;
        wheel_speed_[RIGHT_REAR] = right_side_velocity;
        wheel_speed_[LEFT_FRONT] = left_side_velocity / fl_cos;
        wheel_speed_[LEFT_REAR] = left_side_velocity;
    } else {
        wheel_speed_[RIGHT_FRONT] = 0.0;
        wheel_speed_[RIGHT_REAR] = 0.0;
        wheel_speed_[LEFT_FRONT] = 0.0;
        wheel_speed_[LEFT_REAR] = 0.0;
    }

    for (double &speed : wheel_speed_) {
        speed = clamp_value(speed, -max_wheel_speed_, max_wheel_speed_);
    }
